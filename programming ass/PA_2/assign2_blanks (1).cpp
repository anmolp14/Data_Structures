...
if(NotBalancedNode != NULL)
	{	cout<< "\n\n";
		PrintBST(NotBalancedNode);
		cout<< "\n\n";
		SortedArrayFromBST (NotBalancedNode ,out,0  );
	       	cout << '\n' << out[0] << ' ' ;
		cout << out[1] << ' ';
		cout << out[2]<< ' ';
		NotBalancedNode = BSTFromSortedArray (out , NotBalancedNode -> size );
...
/*	Node * BST ;
	...
	BST = BSTFromSortedArray ( arr , 10  );
	PrintBST ( BST) ;
	SortedArrayFromBST ( BST , out , 0  );
	for( int i=0 ; i< 10 ;i++ )
		cout << out[i];
	if( checkNearBalance(BST ) )
	      cout << "Near";
	if( checkPerfectBalance (BST))
		cout << "perf";
	if(search(BST,1 ) )
		cout << "YES \n";
	else
